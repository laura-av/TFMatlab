...
%figure(1)
subplot(1,3,1)
plot(times, y(:,1), 'b-', 'LineWidth', 1.5)
hold on
%plot(times, S, 'r-','LineWidth', 1.5)
plot(times, y(:,2), 'r-', 'LineWidth', 1.5)
plot(0,I0, 'k.', 'MarkerSize',18)
plot(0,S0, 'c.', 'MarkerSize',18)
legend("I","S","I_0", "S_0",  "Location","best")
title('SIS model'); xlabel('Time'); ylabel('Population');
hold off
% 
% figure(2)
% plot(y(:,1), y(:,2))
...
%figure(3)
subplot(1,3,2)
plot(timesb, yb(:,1), 'b-' ,'LineWidth', 1.5)
hold on
plot(timesb, yb(:,2), 'r-' ,'LineWidth', 1.5)
plot(0,0, 'k.', 'MarkerSize',20)
plot(0,0, 'c.', 'MarkerSize',15)
legend("Derivate I_\beta","Derivate S_\beta","I_{\beta 0}", "S_{\beta0}",  "Location","best")
title('Derivates_\beta SIS'); xlabel('Time'); 
hold off
shg
...
%figure(4)
subplot(1,3,3)
plot(timesg, yg(:,1), 'b-', 'LineWidth', 1.5)
hold on
plot(timesg, yg(:,2), 'r-', 'LineWidth', 1.5)
plot(0,0, 'k.', 'MarkerSize',20)
plot(0,0, 'c.', 'MarkerSize',15)
legend("Derivate I_\gamma","Derivate S_\gamma","I_{\gamma 0}", "S_{\gamma0}",  "Location","best")
title('Derivates_\gamma SIS'); xlabel('Time');
hold off
shg
