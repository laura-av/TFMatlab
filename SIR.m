...
figure(1)
subplot(1,3,1)
plot(times, y(:,1), 'r-', 'LineWidth', 1.8) % Infected
hold on
plot(times, y(:,2), 'g-', 'LineWidth', 1.8) % Recovered
plot(times, y(:,3), 'b--', 'LineWidth', 1.8) % Susceptible
plot(0,I0, 'c.', 'MarkerSize',18)
plot(0,R0, 'k.', 'MarkerSize',18)
plot(0,S0, 'y.', 'MarkerSize',18)
legend("I","R","S","I0","R0", "S0",  "Location","best")
title('SIR Model', 'FontSize',14); xlabel('Time','FontSize',13); ylabel('Population', 'FontSize',13);
hold off
% % Infected - Recovered
% figure(2)
% plot(y(:,1), y(:,2))
...
title('Derivates_\beta SIR','FontSize',14); xlabel('Time', 'FontSize',13);
hold off
shg
...
%figure(6)
subplot(1,3,3)
plot(timesg, yg(:,1),'r-' ,'LineWidth', 1.8)
hold on
plot(timesg, yg(:,2),'g-' ,'LineWidth', 1.8)
plot(timesg, yg(:,3),'b--' ,'LineWidth', 1.8)
plot(0,0, 'y.', 'MarkerSize',18)
plot([0,T], [0,0], '--k', 'LineWidth', 0.5)
legend("Derivate I_\gamma","Derivate R_\gamma","Derivate S_\gamma","I_{\gamma 0}, R_{\gamma0},S_{\gamma0}",  "Location","best")
title('Derivates_\gamma SIR', 'FontSize',14); xlabel('Time','FontSize',13);
hold off
shg
